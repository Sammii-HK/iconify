...
program
  .name('iconify')
  .description('Convert PNG/JPEG images to ICO files and PWA icon sets')
  .version('1.0.0')
  .argument('<input>', 'Input image file path (PNG or JPEG)')
  ...
  .action(async (input: string, options: { outputDir: string; format: string; icoSize: string }) => {
    try {
      // Resolve input path
